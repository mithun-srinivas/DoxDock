...
export default function SplitPdf() {
  ...
  const rangeError = useMemo(() => {
    ...
    for (const g of groups) {
      try {
        ...
        if (!pages.length)
          return `"${g}" has no valid pages (document has ${pageCount}).`
      } catch (err) {
        return err.message
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {file && (
        <>
          ...
          <div className="card space-y-4 p-4">
            <fieldset className="space-y-2">
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="radio"
                  name="mode"
                  checked={mode === 'explode'}
                  onChange={() => setMode('explode')}
                />
            ...
            {mode === 'size' && (
              <label className="block space-y-1">
                ...
                <span className="text-xs text-slate-500">
                  Best-effort: a single page larger than the target becomes its
                  own file (marked <code>-oversize</code>).
                </span>
              </label>
            )}
            {mode === 'bookmarks' && (
              <p className="text-xs text-slate-500">
                Splits at each top-level bookmark. A PDF with no bookmarks
            ...
            {mode === 'ranges' && (
              <label className="block space-y-1">
                ...
                />
                <span className="text-xs text-slate-500">
                  Each group becomes a separate output file.
                </span>
                {rangeError && (
          ...
          <button
            ...
            disabled={
              running ||
              (mode === 'ranges' && (!ranges.trim() || !!rangeError)) ||
              (mode === 'size' && !(Number(sizeMb) > 0))
            }
          >
      ...
      {error && (
        <Note type="error" title="Split failed">
          {error}
        </Note>
      )}
      {result && !running && (
        <ResultGallery
          results={result}
          preview={false}
          zipName={`${baseName(file?.name || 'split')}-split.zip`}
        />
      )}
    </div>
  )
