...
export default function PdfToImages() {
  ...
  const { running, progress, error, result, run, reset, slow, cancel } =
    useJob()
  ...
  const convert = () =>
    run((onProgress) =>
      pdfToImages(file, { format, scale: Number(scale), range }, onProgress),
    )
  ...
  return (
    <div className="space-y-6">
      ...
      {error && (
        <Note type="error" title="Couldn’t convert this PDF">
          {error}
        </Note>
      )}
      {result && !running && (
        <ResultGallery
          results={result}
          zipName={`${file?.name?.replace(/\.pdf$/i, '') || 'pages'}-images.zip`}
        />
      )}
    </div>
  )
