...
export default function CompressPdf() {
  ...
  const { running, progress, error, result, run, reset, cancel, slow } =
    useJob()
  ...
  const go = () =>
    run((p) =>
      compressPdf(
        file,
        { mode, dpi: Number(dpi), quality: Number(quality) },
        p,
      ).then((r) => ({
        ...r,
        blob: r.blob,
  ...
  return (
    <div className="space-y-6">
        ...
        <>
          ...
          <div className="card space-y-4 p-4">
            <label className="block space-y-1">
              ...
              >
                <option value="rasterize">
                  Re-encode pages (best shrink; text becomes non-selectable)
                </option>
                <option value="metadata">
                  Strip metadata only (lossless; keeps text)
                </option>
              </select>
            </label>
      ...
      {error && (
        <Note type="error" title="Compression failed">
          {error}
        </Note>
      )}
      {result && !running && (
        <>
          ...
          {result.after >= result.before && (
            <Note type="warning">
              This PDF didn’t get smaller — it’s likely already optimized or
              text-only. Try the other method or lower settings.
            </Note>
          )}
        </>
      )}
