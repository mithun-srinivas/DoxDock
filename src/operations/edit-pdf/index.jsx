...
function AnnotationBox({
  ...
  const onMove = (e) => {
    ...
    if (w < 6) {
      w = 6
      x = d.mode.includes('w') ? b.x + b.w - 6 : b.x
    }
    if (h < 6) {
      h = 6
      y = d.mode.includes('n') ? b.y + b.h - 6 : b.y
    }
  ...
  let visual = {}
  if (a.type === 'rect')
    visual = { border: `${Math.max(1, a.width * scale)}px solid ${a.color}` }
  else if (a.type === 'ellipse')
    visual = {
      border: `${Math.max(1, a.width * scale)}px solid ${a.color}`,
      borderRadius: '50%',
    }
  else if (a.type === 'highlight')
    visual = { background: a.color, opacity: 0.35 }
  else if (a.type === 'whiteout')
    visual = {
      background: '#fff',
      outline: selected ? '1.5px solid #5ea9ff' : '1px dashed #9aa7b8',
    }

  return (
    <div
      style={{ ...common, width: w, height: h, ...visual }}
      onPointerDown={startMove}
    >
      {a.type === 'image' && (
        <img
...
export default function EditPdf() {
  ...
  const updateAnno = (id, patch) =>
    setAnnos((prev) => prev.map((a) => (a.id === id ? { ...a, ...patch } : a)))
  ...
  const hitTest = (a, p) => {
    ...
    if (a.type === 'line')
      return distToSeg(p.x, p.y, a.x1, a.y1, a.x2, a.y2) <= a.width / 2 + 6
    if (a.type === 'draw') {
      for (let i = 1; i < a.points.length; i++) {
        const A = a.points[i - 1],
          B = a.points[i]
        if (distToSeg(p.x, p.y, A.x, A.y, B.x, B.y) <= a.width / 2 + 6)
          return true
      }
    }
    return false
  ...
  const eraseAt = (p) => {
    setAnnos((prev) => {
      ...
      for (let i = prev.length - 1; i >= 0; i--) {
        const a = prev[i]
        if (a.page === pageIndex && hitTest(a, p)) {
          hitId = a.id
          break
        }
      }
  ...
  const onOverlayUp = () => {
    ...
    setDraft((d) => {
      ...
      const ok =
        ...
        (d.type === 'line' &&
          (Math.abs(d.x2 - d.x1) > 2 || Math.abs(d.y2 - d.y1) > 2)) ||
        (['rect', 'ellipse', 'highlight', 'whiteout'].includes(d.type) &&
          d.w > 3 &&
          d.h > 3)
      if (ok) {
        const { ox, oy, ...clean } = d
  ...
  const finalName = () => {
    let n = (outName || `${baseName(file.name)}-edited`)
      .trim()
      .replace(/[\\/:*?"<>|]/g, '_')
  ...
  const doExport = () =>
    exportJob.run((p) =>
      exportEditedPdf(bytesRef.current, annos, p).then((blob) => ({
        blob,
        filename: finalName(),
      })),
    )
  ...
  const svgAnnos = pageAnnos.filter(
    (a) => a.type === 'draw' || a.type === 'line',
  )
  const boxAnnos = pageAnnos.filter(
    (a) => a.type !== 'draw' && a.type !== 'line',
  )
  ...
  const goodToKnow = (
    <Note type="warning" title="Good to know">
      A visual editor that overlays your additions onto the pages and bakes them
      in on export. It adds new content (text, ink, shapes, images, white-out)
      on top of the PDF — it does not re-flow or edit the document’s existing
      text.
    </Note>
  )
    ...
    <div className="space-y-5">
      {!isStandalone && (
        <div className="flex justify-end">
          <button
            ...
            title="Open the editor in its own window"
          >
      ...
      {!file && (
        <Dropzone
          ...
          hint="Then add text, drawings, highlights, shapes, images, or white-out"
          icon="pencil"
        />
      )}

      {loadJob.running && loadJob.progress && (
        <Progress message="Opening PDF…" />
      )}
      {loadJob.error && (
        <Note type="error" title="Couldn’t open this PDF">
          {loadJob.error}
        </Note>
      )}
        ...
        <>
          ...
          <div className="card flex flex-wrap items-center gap-2 p-2">
              ...
              <>
                ...
                <button
                  ...
                  onClick={() => {
                    const v = !italic
                    setItalic(v)
                    patchSelected({ italic: v })
                  }}
                  className={
                    'flex h-8 w-8 items-center justify-center rounded-lg italic ' +
                    (italic
                      ? 'bg-brand-600 text-white'
                      : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800')
                  }
            ...
            {selectedId && (
              <button
                type="button"
                className="btn-ghost h-9 px-2 text-red-600"
                onClick={() => {
                  commit((p) => p.filter((a) => a.id !== selectedId))
                  setSelectedId(null)
                }}
              >
          ...
          <div
            ref={wrapRef}
            className="flex justify-center overflow-auto rounded-xl bg-slate-200/60 p-4 dark:bg-slate-800/40"
          >
            <div
              ...
              >
                {dims && (
                  <>
                    ...
                    {/* draft box preview */}
                    {draft &&
                      ['rect', 'ellipse', 'highlight', 'whiteout'].includes(
                        draft.type,
                      ) && (
                        <div
                          ...
                          style={{
                            ...
                            border:
                              draft.type === 'rect' || draft.type === 'ellipse'
                                ? `${draft.width * scale}px solid ${draft.color}`
                                : undefined,
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <Icon name="download" className="h-4 w-4" /> Apply edits &amp;
              download
            </button>
            <button
              type="button"
          ...
          {exportJob.running && exportJob.progress && (
            <Progress
              value={exportJob.progress.value}
              message={exportJob.progress.message}
            />
          )}
          {exportJob.error && (
            <Note type="error" title="Export failed">
              {exportJob.error}
            </Note>
          )}
          ...
          {exportJob.result && !exportJob.running && (
            <div className="card space-y-3 p-4">
              <p className="text-sm font-medium text-brand-700 dark:text-brand-300">
                Your edited PDF is ready — name it and download.
              </p>
          ...
          <p className="text-xs text-slate-400">
            Tip: pick a tool, then click or drag on the page. Use Select to
